// this comtains the data we are gonna for AboutMe component and slider component

export const AboutData = [
  {
    count: 1,
    title: "UI / UX Designer",
    img: "/first.svg",
    array: [
      "Information Architecture",
      "Pen and Paper",
      "Figma",
      "Adobe Photoshop",
      "Logo Design",
      "Banner Design",
    ],
  },
  {
    count: 2,
    title: "Front-end Developer",
    img: "/second.svg",
    array: [
      "HTML / CSS3",
      "javaScript (ES6)",
      "ReactJS",
      "NextJS",
      "TailwindCSS",
      "Bootstrap",
      "REST API ",
    ],
  },
  {
    count: 3,
    title: "Back-end Developer",
    img: "/third.svg",
    array: [
      "NodeJS / Express.js",
      "MongoDB",
      "Next.js (API)",
      // "PHP",
      // "WordPress Theme Dev",
      // "WordPress Plugin Dev.",
      // "ASP (Once upon a time)",
    ],
  },
];

// this is for the slider
export const DataArray = [
  {
    title: "Food Order Website Using Reactjs",
    name: "Food Order Website",
    des: "I created this project in 2023 to learn React.js and explore its capabilities in building interactive web applications.",
    des1: "This project was made using, Ractjs and Vanilla CSS",
    tools:["Vanilla CSS", "ReactJs", "Redux ToolKit", "Javascript"],
    images: ["/food1.jpg", "/food2.jpg"],
    link: "https://minorfoodproject.netlify.app/",
  },
  {
    title: "Ecommerce Website",
    name: "Ecommerce Website in MERN stack",
    des: "I build this project to explore the how complete website works.",
    tools:["Tailwind CSS", "ReactJs", "Redux ToolKit", "Javascript", "Materials UI", "React-Router-Dom"],
    des1: "This project was made using, Reactjs, Vanilla CSS, Tailwind CSS, Daisy UI and more..",

    images: ["/ecommerce1.jpg", "/ecommerce2.png", "/ecommerce3.png"],
    link:"https://bibash043.netlify.app/"
  },
  {
    title: "'Sastobazar'  E-commerce Website Project",
    name: "Full Stack E-commerce Project",
    des: "I did this project to explore my knowledge building a full stack ecommerce project .",
    des1: "This project was developed using React.js, Bootstrap, Express.js, and Mongoose. It features a visually appealing user interface on the frontend, seamless integration of RESTful APIs between the backend and frontend, and robust functionalities such as payment processing, user reviews, pagination, and admin management. Additional features include password recovery, email notifications for order delivery, and a comprehensive user experience.",
    tools:["Bootstrap", "ReactJs", "Redux ToolKit", "Javascript","NodeJs", "ExpressJs","MongoDb","ChartJs","Oauth",],
    images: [
      "/sastobazar.jpg",
      "/sastobazar2.jpg",
      "/sastobazar3.jpg",
      "/sastobazar4.jpg",
    ],

    link:"https://sastobazar.onrender.com"
  },
  {
    "title": "'WhatsApp Clone'",
    "name": "Full Stack Chat Application Project",
    "des": "I built this project to deepen my understanding of real-time web applications and explore advanced full-stack development concepts.",
    "des1": "This project was developed using React.js, Redux Toolkit, Node.js, Express.js, and MongoDB. It features real-time messaging, video calling using WebRTC, authentication with JWT, and state management with Redux Persist. Additional functionalities include typing indicators, online statuses, group chats, file sharing, emojis, and secure image storage with Cloudinary. The user interface is intuitive and responsive, providing a seamless experience across all devices.",
    "tools": [
        "Bootstrap",
        "ReactJs",
        "Redux ToolKit",
        "Javascript",
        "NodeJs",
        "ExpressJs",
        "MongoDb",
        "Socket.IO",
        "WebRTC",
        "Cloudinary"
    ],
    images: [
      "/chat.jpg",
      "/chat2.png",
      "/chat3.jpg",
    ],
    link:"https://nepalconnect.onrender.com/"
  },
{
  "title": "News Website",
  "name": "Full Stack News Site Project",
  "des": "I built this project to practice full-stack development and strengthen my skills in building modern, responsive web applications.",
  "des1": "This project was developed using React.js, Tailwind CSS, Node.js, Express.js, and MongoDB. It features user authentication with JWT, secure API integration, and efficient state management with Redux Toolkit. The website allows users to browse, read, and explore news articles across different categories with a clean and responsive user interface. Tailwind CSS ensures a modern design, while MongoDB and Express.js handle robust backend operations. The project provides a smooth and optimized user experience across all devices.",
  "tools": [
    "Tailwind CSS",
    "React.js",
    "Redux Toolkit",
    "JavaScript",
    "Node.js",
    "Express.js",
    "MongoDB"
  ],
  "images": [
    "/news.png",
  ],
  "link": "https://bhandaphor.vercel.app/"
},
{
  "title": "Consultancy Website",
  "name": "Study Abroad Consultancy Portal",
  "des": "I built this project to practice full-stack development and gain hands-on experience with building a professional consultancy portal.",
  "des1": "This project was developed using React.js, Tailwind CSS, Node.js, Express.js, and MongoDB. It includes an appointment booking system with calendar integration, lead tracking, and an admin dashboard for content management. The platform provides a seamless experience for students to connect with consultancy services, while the responsive UI ensures smooth usability across all devices.",
  "tools": [
    "Tailwind CSS",
    "React.js",
    "Redux Toolkit",
    "JavaScript",
  ],
  "images": [
    "/consultancy.png"
  ],
  "link": "https://consultancy-abroad.vercel.app/"
},
{
  "title": "Restaurant Website",
  "name": "Online Restaurant Project",
  "des": "I built this project to enhance my full-stack development skills and explore features for modern restaurant management systems.",
  "des1": "This project was developed using React.js, Tailwind CSS, Node.js, Express.js, and MongoDB. It includes an online food ordering system, menu browsing with categories, table reservation, and an admin dashboard for managing orders and menu items. The platform ensures a smooth user experience with a responsive UI, making it easy for customers to explore and place orders from any device.",
  "tools": [
    "Tailwind CSS",
    "React.js",
    "Redux Toolkit",
    "JavaScript"
  ],
  "images": [
    "/restaurant.png"
  ],
  "link": "https://restaurant-frontend-pearl-pi.vercel.app/"
},
{
  "title": "IT Solutions Website",
  "name": "Tech Solutions Portal",
  "des": "I built this project to strengthen my full-stack MERN skills by creating a professional IT solutions platform for businesses and clients.",
  "des1": "This project was developed using React.js, Tailwind CSS, Node.js, Express.js, and MongoDB. It features service listings for IT solutions such as software development, cloud services, and digital consultancy. Clients can request quotes, book consultations, and track project progress. The admin dashboard allows management of services, client inquiries, and project details. With a responsive and modern UI, the platform delivers a professional experience across all devices.",
  "tools": [
    "Tailwind CSS",
    "React.js",
    "Redux Toolkit",
    "JavaScript",
    "Node.js",
    "Express.js",
    "MongoDB"
  ],
  "images": [
    "/nexdhara.png"
  ],
  "link": "https://nexdhara-website.vercel.app"
}


];
